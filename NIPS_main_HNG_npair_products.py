    ...
    if FLAGS.Apply_HDML:
        ...
        with tf.name_scope('Loss'):
            def get_generator(name):
                return 'batch_normalization' not in name and 'Generator' in name

            g_wdLoss = FLAGS.Regular_factor * tf.add_n(
                [tf.nn.l2_loss(v) for v in tf.trainable_variables() if get_generator(v.name)]
            )
            ...
            J_metric = J_m + J_syn
            J_metric = tf.cond(tf.is_nan(J_metric), lambda: tf.constant(0.), lambda: J_metric)
            ...
            J_gen = J_recon + J_soft + g_wdLoss

    if FLAGS.Apply_HDML:
        c_train_step = nn_Ops.training(loss=J_metric, lr=lr, var_scope='Classifier', g_truncate=True)
        g_train_step = nn_Ops.training(loss=J_gen, lr=FLAGS.lr_gen, var_scope='Generator', g_truncate=True)
        s_train_step = nn_Ops.training(loss=cross_entropy, lr=FLAGS.s_lr, var_scope='Softmax_classifier', g_truncate=True)
    else:
        ...
        for batch in tqdm(copy.copy(epoch_iterator)):
            ...
            if step >= 4000:
                ...
                if not FLAGS.Apply_HDML:
                    ...
                    print('max %d' % np.max(Label_raw))
                    print('min %d' % np.min(Label_raw))
                ...
                if step % bp_epoch == 0:
                    ...
                    recalls_te = evaluation.products_Evaluation(
